...
それを踏まえて、(9)の等式`=`の両辺をよく見比べてみてください。異なっているのは`w1 <> (w2 <> w3)`と`(w1 <> w2) <> w3)`の箇所だけですね！つまり、`Writer` `Monad`における`>>=`の結合則は、`w1 <> (w2 <> w3)`と`(w1 <> w2) <> w3)`が等しいから、すなわち`Monoid`における`<>`の結合則が成り立つからこそ成立するのです。これがまさしく「`Monoid`と`Writer`の切っても切り離せない関係」なのです！
