...
例えば、以下の単純なソースの場合について例を示します。  
少し恣意的な例ですが、関数f3の第１引数は正格（かつ関数内で使用されている）、第2引数は非性格（かつ関数内で使用されている）、第3引数は非性格（かつ関数内で使用されていない）という場合の例です。
...
第2引数は、`<L,1*U(U)>`の部分により表されています。 `L`は引数が非性格（non-strict）であることを示しています。 `1*U(U)`は、引数が関数内で使用(Use)されていることを示しています。  
第3引数は、`<L,A>`の部分により表されています。 `L`は引数が非性格（non-strict）であることを示しています。 `A`は、引数が関数内で不使用（Absence）であることを示しています。
...
実行時の空間プロファイルを出力するには、実行オブジェクトの起動時に[`+RTS`][rts_+RTS] [`-xc`][rts_-xc]オプションを指定します。 （[`-xc`][rts_-xc]がスタックトレース出力のためのオプションです。）
